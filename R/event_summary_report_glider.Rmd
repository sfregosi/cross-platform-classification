...
params: 
  ...
  trip: LL062 # 5 char string e.g., LL061, doesn't need quotations
  rec: FR04_DL88 # 9 char string e.g., FR45_DL84, doesn't need quotations
  ...
  refSpecList: !r c('meanSpecGm', 'refSpecPc_LLHARP') # use !r to call r then 
  refSpecSp: !r c('Gm', 'Pc')
---

## Trip/recorder: `r params$trip` `r params$rec`
...
# name project
ProjID <- 'Longline Acoustic Monitoring'

# combine trip, recorder, pg version (all defined in YAML) to single run string
pgRunStr <- paste0(params$pgver, '_llharp_banter_', params$trip, '_', params$rec)
trStr <- paste0(params$trip, '_', params$rec)

# select path to upper pamguard folder
# path_pg = choose.dir(default = "", caption = "Select path to pamguard folder")
# or predefined in YAML
# NB using //piccrp4nas (raw network path) causes error in updateFiles(), using
# params$drive works fine

# define subfolder paths based on selected analysis folder and trip strings
path_to_db <- file.path(params$path_pg, 'databases', params$trip)
path_to_binaries <- file.path(params$path_pg, 'binaries', params$trip, pgRunStr)

# assemble some file names
# pamguard database
dbFile <- file.path(path_to_db, paste0(pgRunStr, '.sqlite3'))
# already processed acoustic study 'dets' file
detsFile <- file.path(params$path_ed, 'dets', 
                      paste0(params$pgver, '_', trStr, '_dets.rda'))
detsFiltFile <- file.path(params$path_ed, 'dets_filtered', 
                          paste0(params$pgver, '_', trStr, '_detsFilt.rda'))
# specify calibration file
# for LL060 onward use: '//picqueenfish/psd2/crp/LLHARP/calibration/
#                             RevN_type4_110516-07_manual.tf'
